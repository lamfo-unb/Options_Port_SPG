...
for _ep in range(206,MAX_EPISODES):
...
#    state = torch.from_numpy(state).float()
    ...
    #    action1 = trainer.get_exploitation_action(state)
        ...
        reward=RetornoT(OP.StateList[OP.StateCont],OP.StateList[-1],OP.Capital1,action,OP.last_Q)[2]

#        if r==1:
        StrategiFlow(OP.StateList[OP.StateCont],action)

        new_observation, rewardF, done, info =OP.Action(action)
